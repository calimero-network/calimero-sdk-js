...
// NOTE: context membership/lifecycle (add/remove/is_member/members/create/
// delete/resolve_alias) are intentionally NOT imported. The node does not
// provide these host functions, so importing them makes the module fail to
// instantiate ("unknown import"). Context membership is managed out-of-band
// via the admin API / merobox workflows, not from inside the WASM guest.
...
// NOTE: context membership/lifecycle wrappers removed — the node provides no
// matching host imports (see the extern block above).
...
void js_add_calimero_host_functions(JSContext *ctx) {
  ...
  // Context membership/lifecycle bindings removed — no matching host imports.
